...
async function loadDienstplan(jahr, monat, force = 0) {
  ...
  try {
    // hier benutzen wir die Route:
    //   GET /api/dienstplan?jahr=YYYY&monat=MM&force=0|1
    ...
    // data ist bereits das komplette Plan-Objekt (jahr, monat, filialen, ...)
...
function renderFilialeMatrix(fBlock, gesamtData) {
  ...
  titleSpan.textContent = `${name} (ID: ${filiale.id})`;
  ...
  // WICHTIG:
  // Wir nehmen ALLE Einsätze, deren filialeId = diese Filiale ist.
  // Egal, welche Heimatfiliale der MA hat → so gehen Springer/fremde MA wieder.
  gesamtData.filialen.forEach(f => {
    (f.plan || []).forEach(tag => {
      const datum = tag.datum;

      (tag.einsatz || []).forEach(e => {
        // NUR Einsätze in dieser Filiale anzeigen
        if (Number(e.filialeId) !== Number(filiale.id)) return;
  ...
  mitarbeiterListe.forEach(m => {
    ...
    dates.forEach(datum => {
      ...
      const farbe = einsatz.farbe || filiale.farbe;
