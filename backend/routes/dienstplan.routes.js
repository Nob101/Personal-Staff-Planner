...
router.post("/shift", async (req, res) => {
  try {
    ...
    const before = await dienstplanRepo.getById(id);
    if (!before) {
    ...
    const updated = await dienstplanRepo.dienstShift(id, neuTyp);
    if (!updated) {
      return res.status(404).json({ error: "Dienst nicht gefunden." });
    }
    ...
    let stunden = null;
    if (delta !== 0) {
      stunden = await stundenRepo.updateIstStunden(before.mnr, before.jahr, before.monat, delta);
    }
